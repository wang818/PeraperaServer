...
async def download_youtube_audio(url: str = "https://www.youtube.com/watch?v=GUxIotkN2zg"):
    ...
    try:
        ...
        # 配置 yt-dlp 选项 - 使用 mweb 客户端
        ydl_opts = {
            'format': 'bestaudio/best',
            'outtmpl': output_path,
            'extractor_args': {
                'youtube': {
                    'player_client': ['mweb'],  # 移动网页客户端，不需要 cookies
                }
            },
            'postprocessors': [{
...
@router.get("/yt_audio_test")
async def download_youtube_audio_test(url: str = "https://www.youtube.com/watch?v=GUxIotkN2zg"):
    """测试使用 mweb 客户端下载 YouTube 音频（不需要 cookies）"""
    try:
        # 创建临时目录
        temp_dir = tempfile.mkdtemp()
        output_path = os.path.join(temp_dir, "test")

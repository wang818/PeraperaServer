...
TRANSLATIONS: Dict[str, Dict[str, str]] = {
    ...
    "email_captcha_body": {
        "en": "You are logging in to {app_name}. Your verification code is: {captcha}",
        "ar": "أنت تسجل الدخول إلى {app_name}. رمز التحقق الخاص بك هو: {captcha}",
        "de": "Sie melden sich bei {app_name} an. Ihr Bestätigungscode lautet: {captcha}",
        "es": "Está iniciando sesión en {app_name}. Su código de verificación es: {captcha}",
        "fil": "Nagla-log in ka sa {app_name}. Ang iyong code ng pagpapatunay ay: {captcha}",
        "fr": "Vous vous connectez à {app_name}. Votre code de vérification est: {captcha}",
        "id": "Anda sedang masuk ke {app_name}. Kode verifikasi Anda adalah: {captcha}",
        "ja": "{app_name}にログインしています。認証コードは: {captcha}",
        "ko": "{app_name}에 로그인하고 있습니다. 인증 코드는: {captcha}",
        "ms": "Anda sedang log masuk ke {app_name}. Kod pengesahan anda ialah: {captcha}",
        "my": "သင္သည် {app_name} သို႔ ဝင္ေရာက္ေနပါသည္။ သင့္၏ အတည္ျပဳကုဒ္မွာ: {captcha}",
        "pl": "Logujesz się do {app_name}. Twój kod weryfikacyjny to: {captcha}",
        "pt": "Você está fazendo login no {app_name}. Seu código de verificação é: {captcha}",
        "ru": "Вы входите в {app_name}. Ваш код подтверждения: {captcha}",
        "th": "คุณกำลังเข้าสู่ระบบ {app_name} รหัสยืนยันของคุณคือ: {captcha}",
        "tr": "{app_name} hesabınıza giriş yapıyorsunuz. Doğrulama kodunuz: {captcha}",
        "vi": "Bạn đang đăng nhập vào {app_name}. Mã xác nhận của bạn là: {captcha}",
        "zh-CN": "您正在登录{app_name}，登录验证码为：{captcha}",
        "zh-Hant": "您正在登入{app_name}，登入驗證碼為：{captcha}"
    },
